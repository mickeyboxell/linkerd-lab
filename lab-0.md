...
![first sign in](.gitbook/assets/firstsignin.png)
...
![redwood sign in](.gitbook/assets/redwoodsignin.png)
...
![account name](.gitbook/assets/accountname.png)
...
2. In the Console, open the navigation menu. Under **Solutions and Platform**, go to **Developer Services** and click **Container Clusters**.
3. Choose a **Compartment** you have permission to work in, and in which you want to create both the new cluster and the associated network resources.
4. On the **Cluster List** page, click **Create Cluster**.
5. Either just accept the default configuration details for the new cluster, or specify alternatives as follows:
...
7. The **Create Node Pool** panel also contains some node pool properties that you can change, but which have been given sensible defaults. Either just accept all the default configuration details and skip ahead to the next step to create the cluster immediately, or specify alternatives as follows: Either accept the default configuration details for the node pool, or specify alternatives in the Create Node Pool panel as follows:
...
8. Either accept the defaults for the remaining cluster details, or specify alternatives in the **Additional Add Ons** panel as follows:
...
9. Click **Create** to create the new network resources and the new cluster. Container Engine for Kubernetes starts creating the following resources:

   * the network resources \(such as the VCN, internet gateway, NAT gateway, route tables, security lists, private subnets\), named oke--quick--
...
16. Test your kubeconfig and `kubectl` are configured correctly with:
...
[Oracle Cloud Site](https://github.com/mickeyboxell/linkerd-lab/tree/223ca8ff5339df320ad2249d5ce373318f5a40ef/cloud.oracle.com)
